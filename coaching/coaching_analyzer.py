...
「本日ご入会の場合は、入会金と事務手数料が0円になります。
かなりお得に始めていただけます。」
※料金の事実(厳守): 本日入会で無料になるのは「入会金」と「事務手数料」だけ。
  初回体験料金(今日の施術料金)はかかる。「今日の施術料金も0円」「今日の施術が無料」とは絶対に書かない。
...
ヒアリング★スコアは この結果 + 聞き方の質 で決定する(配点は後述の【評価項目】を参照)。
※ 生活習慣(食事・アルコール・水分・睡眠・運動・歩数・体重増減・お風呂)は全部聞く必要はない。
  お客様の悩みに関係するものを1〜2個でも聞けていれば十分に評価する。
...
【評価項目(各1〜5の★スコア)】
1. ヒアリング: 【聞けた項目の点数】で基準の★を決め、そのあと【聞き方の質】で±1する(下限★1・上限★5)

   ■ STEP1 基準の★(項目を点数に換算して決める)
   【A】重要8項目(各1点・最大8点)
     悩みトップ3 / 最優先深掘り / 目的目標 / 仕事 / 既往歴 / セルフケア / 美容医療 / プランニング
   【B】生活習慣8項目(食事 / アルコール / 水分 / 睡眠 / 運動 / 歩数 / 体重増減 / お風呂)
     聞けた個数を点に換算する: 0個=0点、1個=1点、2個=2点、3個以上=3点(最大3点)
     ※ 生活習慣は全部聞くものではない。悩みに関係するものを1〜2個聞けていれば十分な加点とする
   合計点(最大11点)で基準の★を決める
   - 10-11点 → ★5
   - 8-9点  → ★4
   - 6-7点  → ★3
   - 4-5点  → ★2
   - 0-3点  → ★1
   ※ ただし「最優先深掘り」と「目的目標」が両方とも聞けていない場合、基準の★は最大★3までとする
     (悩みの原因と目的を聞けていないカウンセリングを高評価にしないため)

   ■ STEP2 聞き方の質で調整(項目数を稼ぐだけの「尋問」を高く評価しないため)
   ▼ -1 する(質問攻め・尋問になっている): 次のうち2つ以上に当てはまるとき
     ・お客様の答えに共感も相槌も返さないまま、次の質問に移る場面が続く
     ・チェックリストを読み上げるような一問一答が続き、会話として流れていない
     ・聞いた内容を施術中の説明や提案に一切つなげず、「聞いて終わり」になっている
     ・なぜ聞くのかを伝えないまま、体重・飲酒・睡眠などの生活習慣を立て続けに質問している
     ・お客様の返答が「はい」「そうですね」など短い相槌ばかりで、会話が広がっていない
   ▼ +1 する(会話として自然に聞けている): 次のうち2つ以上に当てはまるとき
     ・お客様の答えに共感・受け止めを返してから次の質問に入っている
     ・お客様の答えを受けて、予定になかった深掘りができている
     ・聞いた内容を施術中の説明・提案・クロージングに結びつけている
     ・「〜のためにお聞きするのですが」と質問の意図を添えている
   ▼ どちらにも当てはまらない場合は、基準の★のまま
   ※ 7つの感(親近感・安心感)や悩み深掘りの質は、この STEP2 の判断材料として使う
   ※ ★を上下させたときは、その理由を good_points / improvements のどちらかに必ず一言書く
     (例:「項目はよく聞けていましたが、質問が続けて並び会話が途切れていたため★を1つ下げました」)
...
【FB生成時の必須チェック】
- ヒアリング項目のうち「聞けてなかった項目」を明示し、次回ヒアリングで聞くよう促す。
  ただし生活習慣(食事・アルコール・水分・睡眠・運動・歩数・体重増減・お風呂)は全部を並べない。
  今回の悩みに関係が深いものを1〜2個だけ挙げて「次はこれも聞いてみよう」と促す
- ヒアリング★を達成数から上下させた場合は、その理由(質問攻めになっていた/会話として自然に聞けていた)を必ず書く
- 「7つの感」のどれが弱かったかを必ず1つ以上指摘
- 危機感トーク・3ヶ月提案・入会金無料トークが入っていたか言及
- 「ハリナチュレじゃなくてもいい」フレーズが入っていたか確認
- 料金の説明・書き換え例では「入会金と事務手数料が無料(初回体験料金はかかる)」を守る。
  「今日の施術が無料/0円」という誤った案内を書かない(スタッフがそう言っていたら誤りとして指摘する)
...
def send_slack_notifications(staff_name: str, session_date, result: dict,
    ...
    # スマホのSlackだと1行に並べると途中で折り返して読みにくいので、1項目ずつ改行する
